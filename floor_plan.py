from objects import Floor, Wall, ResourceSource, CBoard, Fryer, CBelt, TrashCan, Floors, Walls, CounterTops, CounterTop
from constants import SIZE, FISH_CRATE_ICON, POTATO_CRATE_ICON, PLATE_CRATE, CB_R_ICON, FRYER_R_ICON, COLS, ROWS, START_X, START_Y
...
CBelt1 = CBelt(menu=Menu, position=(START_X+SIZE/2 + CBelt1_x*SIZE, START_Y+SIZE/2 + CBelt1_y*SIZE))

CBelt2_x, CBelt2_y = CBelt1_x+1, CBelt1_y
floor_plan_matrix[CBelt2_y][CBelt2_x] = 3
CBelt2 = CBelt(menu=Menu, position=(START_X+SIZE/2 + CBelt2_x*SIZE, START_Y+SIZE/2 + CBelt2_y*SIZE))

CBelt3_x, CBelt3_y = CBelt1_x, CBelt1_y-1
floor_plan_matrix[CBelt3_y][CBelt3_x] = 3
CBelt3 = CBelt(menu=Menu, position=(START_X+SIZE/2 + CBelt3_x*SIZE, START_Y+SIZE/2 + CBelt3_y*SIZE))

CBelt4_x, CBelt4_y = CBelt1_x+1, CBelt1_y-1
floor_plan_matrix[CBelt4_y][CBelt4_x] = 3
CBelt4 = CBelt(menu=Menu, position=(START_X+SIZE/2 + CBelt4_x*SIZE, START_Y+SIZE/2 + CBelt4_y*SIZE))

Trash_x, Trash_y = CBelt2_x+1, CBelt2_y
floor_plan_matrix[Trash_y][Trash_x] = 3
Trash = TrashCan(menu=Menu, position=(START_X+SIZE/2 + Trash_x*SIZE, START_Y+SIZE/2 + Trash_y*SIZE))
